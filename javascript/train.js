...
var trainStart = snapshot.val().train_first;
console.log(trainStart);
var convertedTrainStart = moment(trainStart, "hh:mm").subtract(1, "years");
var timeDifference = moment().diff(moment(convertedTrainStart),"minutes");
console.log(timeDifference);
var timeApart = timeDifference % snapshot.val().train_freq;
var minutesUntilNextTrain = snapshot.val().train_freq - timeApart;
var nextArrival = moment().add(minutesUntilNextTrain, "m").format("LT");

$('#train_schedule').append('<tr>'
+'<td>'+ snapshot.val().train_name
+'</td><td>'+ snapshot.val().train_dest
+'</td><td>'+ snapshot.val().train_freq
+'</td><td>'+ nextArrival
+'</td><td>'+ minutesUntilNextTrain
+'</td></tr>')
